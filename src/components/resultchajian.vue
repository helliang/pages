 ...
 * @Last Modified time: 2018-08-20 11:38:53
 */
<template>
    <div class="result">
        <!-- 显示结果 -->
...
export default {
    ...
    methods: {
        init() {
            ...
            console.log(this.coffeeType.record.length);
            // 把缓存下来的数据放入显示页面的数组中
        ...
        scrollLoding() {
            ...
            if(self.coffeeType.record.length >= 20){
                console.log( console.log(this.coffeeType.record))
                // let nmuber = 1;
                    // 设置一个开关来避免重负请求数据
                    if (self.scrollWatch == true) {
                        ...
                        // 将开关关闭
                            ...
                            let pageNmuber = 2;
                            // 判断上一个页面返回来的数据是否大于20条
                            $(document.body).infinite().on("infinite", function() {
                                  $(document.body).destroyInfinite()
                                // console.log("页面",nmuber++)
                                let params = {
                                    pageSize: 20,
                                    sysUnit:44,
                                    currentPage: pageNmuber,  // 滚动加载页面数量 
                                    ...
                                    // 如果开关打开则加载数据
                                        ...
                                        self.$http.get(self.apis.baseURL,{params}) .then(function(response) {
                                            ...
                                            self.showLoding = false;
                                                // pagesNmu++;
                                                pageNmuber++;
                                                console.log('请求成功的页面数量',params.currentPage)
                                                    self.numLen = response.data.record.length;
                                                // console.log(numLen)
                                            // if(response.data.record.length >= 20){
    ...
    data() {
        return {
            ...
            //获取每次下拉返回的数量 
            numLen:'',
            scrollWatch :true,//滚动开关
            showTip:false,
            tip:'已加载完所有数据!'
        };
    },
     beforeRouteLeave (to, from, next) {
         if(from.path == "/result"){//离开的会停止加载
          $(document.body).destroyInfinite();
